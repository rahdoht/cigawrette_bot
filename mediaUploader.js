...
import { fullyEncodeURI, oauth } from "./utils.js";
...
export class TwitterMediaUploader {
  ...
  async init(data) {
    const r = data.map(async ({ path, type }, i) => {
      ...
      return request({
        ...
        .catch((e) =>
          console.error("TwitterMediaUploader: error with init", e.message)
        );
    });
  ...
  async append(chunk, mediaId, i = 0) {
    ...
    return request({
      ...
      console.error(
        "TwitterMediaUploader: error with append segment " + i,
        e.message
      )
    );
  }
  ...
  async finalize(mediaId) {
    ...
    return request({
      ...
      console.error("TwitterMediaUploader: error with finalizing", e.message)
    );
  }
  ...
  async tweet(status) {
    ...
    return request({
      ...
      console.error("TwitterMediaUploader: error with tweet", e.message)
    );
  }
  ...
  async getFile(i) {
    if (!this.mediaPaths[i].startsWith("http"))
      return fs.readFileSync(this.mediaPaths[i]);
    ...
    return request({
      ...
      console.error(
        "error getting external media file from " + this.mediaPath,
        e.message
      )
    );
  }
